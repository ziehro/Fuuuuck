...
class _AddBeachScreenState extends State<AddBeachScreen> {
  ...
  Future<void> _getCurrentLocationAndGeocode() async {
    ...
    try {
      ...
      if (permission == LocationPermission.whileInUse || permission == LocationPermission.always) {
        ...
        setState(() {
          // Add a debug print to see the raw values
          debugPrint('Geolocator raw position: lat=${position.latitude}, lon=${position.longitude}');

          // CRITICAL: Check for a swap and correct if it happened.
          // This is a defensive fix for the unusual behavior on your device.
          // If latitude is outside the valid range, assume it's the longitude.
          if (position.latitude < -90 || position.latitude > 90) {
            _currentLocation = LatLng(position.longitude, position.latitude);
            debugPrint('Lat/Lon swapped to: lat=${_currentLocation!.latitude}, lon=${_currentLocation!.longitude}');
          } else {
            _currentLocation = LatLng(position.latitude, position.longitude);
            debugPrint('Lat/Lon used as-is: lat=${_currentLocation!.latitude}, lon=${_currentLocation!.longitude}');
          }
        });
  ...
  Future<void> _saveNewBeach() async {
    ...
    // --- THIS IS THE NEW DEBUG PRINT ---
    debugPrint('Final values before save: lat=${_currentLocation!.latitude}, lon=${_currentLocation!.longitude}');
    // --- END DEBUG PRINT ---
    ...
    try {
      ...
      final initialBeach = Beach(
        ...
        geohash: geoHasher.encode(_currentLocation!.latitude, _currentLocation!.longitude, precision: 9),
